...
% In this release, Some bugs are fixed by Zhangquan liao 
% 1. "if nargin==6" -> "if nargin==8"
% 2. Three missing situations "if tmp1==0 && tmp2~=0 && tmp3~=0",
%							  "if tmp1~=0 && tmp2==0 && tmp3~=0",
%                             and "if tmp1~=0 && tmp2~=0 && tmp3==0" are added
% Nov, 2022
...
if mode==1 %possible for other patching options
    ...
    if tmp1==0 && tmp2~=0 && tmp3~=0
        A=zeros(n1,n2+s2-tmp2,n3+s3-tmp3);
        mask=zeros(n1,n2+s2-tmp2,n3+s3-tmp3);
    end
    ...
    if tmp1~=0 && tmp2==0 && tmp3~=0
        A=zeros(n1+s1-tmp1,n2,n3+s3-tmp3);
        mask=zeros(n1+s1-tmp1,n2,n3+s3-tmp3);
    end
